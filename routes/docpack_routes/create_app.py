...
async def action_create_app(form,field,R):
  ...
  if not(service_id):
    ...
    #print(f'{service=} {summ=}')
    if not(service):
      form.errors.append('Услуга не найдена')
    elif service['min_prepaid']>summ:
      form.errors.append(f"Минимальная сумма предоплаты: {service['min_prepaid']}")
  ...
  if not(docpack_id):
    ...
    if manager['login'] not in ('admin','pzm') and (form.ov.get('brand_id') and form.ov.get('brand_id') == 1 and docpack.get('ur_lico_id') and docpack.get('ur_lico_id') == 28):
        form.errors.append(f'Запрещено создавать приложения на юр. лицо ООО "БЕСТ КОНСАЛТ"')
