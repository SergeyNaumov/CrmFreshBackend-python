...
async def load_app(app_id,ext:str,need_print: int, debug=0):
    ...
    docpack = await db.query(
        query='select * from docpack dp left join dogovor_app da on da.dogovor_id=dp.id where da.id = %s',
        values=[app_id], onerow=1, debug=1
    )
    ...
    dp = await db.query(
        query=f'''
            SELECT
                ...
                app.postpaid_bill_id b_bill_id,
                if(app.postpaid_bill_id, concat('счёт №',app.postpaid_bill_id), '') b_bill_header,
    ...
    if dp['with_nds']:
        nds_percent = 5
        app_summ=float(dp['app_summ'])
        app_summ_without_nds = round(app_summ / (1 + nds_percent / 100), 2)
        app_summ_nds = round(app_summ - app_summ_without_nds, 2)
        dp['nds_text_info']=f' в т.ч. НДС {nds_percent}% {app_summ_nds}'

        app_summ_post=float(dp['app_summ_post'])
        app_summ_post_without_nds = round(app_summ_post / (1 + nds_percent / 100), 2)
        app_summ_post_nds = round(app_summ_post- app_summ_post_without_nds, 2)
        dp['nds_text_post_info']=f' в т.ч. НДС {nds_percent}% {app_summ_post_nds}'
    else:
        dp['nds_text_info']=''
        dp['nds_text_post_info']=''
