...
def process_bill_list(form, _list):
    ...
    for b in _list:
      ...
      if perm.get('admin_paids') or perm.get('accountant'):
          # Если это менеджер платежей или бухгалтер
          b['make_edit_summ']=True
      else:
          b['make_edit_summ']=False
      # Ранее был доступ у менеджера к исправлению своих счетов, теперь нет
...
async def get_bills(form,field, R):
  ...
  if dogovor_id:=R.get('dogovor_id'):
      ...
      apps_list = await db.query(
        query="""
          select
            ...
            a.attach,
            concat( if(uc.regnumber,uc.regnumber,'-'),' от ',
            DATE_FORMAT(uc.born,%s) ) sr_name, u.id user_id, '' error
          from
            dogovor_app a
      ...
      for a in apps_list:
        ...
        registered=str(a['registered'])
        if a['attach']:
          a['attach']=f"/files/dogovor_app-scan/{'/'.join(registered.split('-'))}/{a['attach']}"

        a['fields'] = await db.query(
          query=f"""
            SELECT
              wt.id, sv.label name, wt.value, '' edited
            FROM
        ...
        a['registered']=date_to_rus(registered).replace(' 00:00:00','')
  ...
  if dp:
    ...
    if dp['old'] or perm.get('admin_paids') or perm.get('manager_and_summ_bill_change'): # or len(apps_list)
