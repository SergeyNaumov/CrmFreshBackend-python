...
async def id_filter_code(form,field,row):
        return f'{row["wt__id"]}<br><small style="white-space: nowrap;">{row["wt__registered"]}</small>'

async def without_ftp_before_code(form,field):
  if form.script=='admin_table':
    field['value']=[1]

async def registered_before_code(form,field):
  if form.script=='admin_table':
    field['value']=''
  # 'default_off':1,
  #pass
async def link_filter_code(form,field,row):
...
async def api_links_filter_code(form,field,row):
  v=row['wt__api_links']
  if v:
    return f'<a href="{v}" target="_blank">отправить</a>'
  return '-'
  
async def ts_before_code(form,field):
  if form.script=='admin_table':
    cd=cur_date()
...
async def is_double_filter_code(form,field,row):
   form.pre(field)
