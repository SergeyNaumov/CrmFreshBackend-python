...
def manager_id_before_code(form, field):
  ...
  #form.pre(field)
def manager_fin_before_code(form,field):
  
  if form.manager['login'] in ('admin','pzm','akulov','sed') or form.is_manager_fin_owner:
    field['read_only']=False
...
def group_id_before_code(form,field):
  ...
  #if form.id and form.ov and form.ov.get('mgu__owner') == form.manager.get('id'):
    # руководителю группы юристов 
  #  field['read_only']=False
  ...
  if form.script=='edit_form':
    ...
    if user_id:
      html=f'''
        ...
        <b>Наименование компании:</b> {form.ov["firm"]}<br>
        Карточка ОП: <a href="/edit_form/user/{user_id}" target="_blank">{config["system_url"]}edit_form/user/{user_id}</a><br>
      '''


      if logo:=form.ov.get('brand_logo'):
        html+=f'<img src="/files/logo/{logo}">'
      elif form.ov['brand']:
...
def firm_filter_code(form,field,row):
  ofp_link=f'<a href="/edit_form/user_fin/{row["wt__id"]}" target="_blank">в карту фин. услуг</a>'
...
async def memo_after_add(form,field,data):
  ...
  to={7576: True, 4201: True} # pzm,sed
  to[manager_id]=True



  for recipent_id in [form.ov['manager_id'], form.ov['manager_fin'], form.ov['underwriter']]:
    if recipent_id:
  ...
  if len(to_emails):
    ...
    if len(last_comments)>1:
      ...
      send_mes(
        ...
        subject=f"Новый комментарий, совместная фин. услуг {form.ov['firm']} / {form.ov['product_label']}",
      ...
      send_mes(
        ...
        #to='svcomplex@yandex.ru',#
        to=','.join(to_emails.keys()),
        subject=f"Первый комментарий, совместная работа фин. услуг / {form.ov['firm']}", # / {form.ov['product_label']}
        message=f"Наименование компании: {form.ov['link']}<br>"+\
            f"Менеджер: {form.manager['name']}<br>"+\
            f"Комментарий: {data['comment']}<br>"
...
async def product_before_code(form,field):
  # Если вид продукта указан, то запрещаем редактировать
  #form.pre(field['value'])
  if field['value'] and field['value']!='0':
    field['read_only']=True

def underwriter_before_code(form,field):
  if form.manager['login'] in ('sed','admin','akulov','pzm') or form.is_manager_fin_owner:
    field['read_only']=False
  if form.is_underwriter:
    field['read_only']=False
...
events={
  ...
  'manager_fin':{
    'before_code':manager_fin_before_code
  },
  'underwriter':{
    'before_code':underwriter_before_code
  },
  # 'manager_to2':{
