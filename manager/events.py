...
async def events_permissions(form):
    ...
    if not(perm['manager_access']) and not(perm['manager_all_edit']) and not(manager.get('is_owner') and len(manager['CHILD_GROUPS'])):
    ...
    if form.id:
      ...
      if not perm['manager_all_edit'] and ov and manager.get('CHILD_GROUPS') and ov.get('group_id') not in manager.get('CHILD_GROUPS'):
        form.errors.append('доступ запрещён!')
